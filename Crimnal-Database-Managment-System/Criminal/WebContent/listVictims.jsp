...
<h1 style="color: red">ALL Victims</h1>
<table cellpading="5px" style="width:60%; height: 100px;text-align: left";>
  <sql:query var="result93" dataSource="${snapshot}">select * from victims natural join fir_status where stat='1';</sql:query>
   <tr><th>SNO</th><th>Victims</th></tr>
   <% i=1; %>
   <c:if test="${result93.rowCount!=0}">
   <c:forEach var="row" items="${result93.rows}">
   <tr><td><%= i++ %></th><td><a href="viewVictims.jsp?address=<c:out value="${row.address}"/>&name=<c:out value="${row.name}"/>">Victim&nbspNAME:<c:out value="${row.name}"/>&nbspAddress:<c:out value="${row.address}"/></a><br>
<br></td></tr>
   </c:forEach>
   </c:if>
  </table>
