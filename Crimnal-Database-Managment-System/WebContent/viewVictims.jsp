...
<sql:query var="result" dataSource="${snapshot}">
	select * from victims where name='<%=StringEscapeUtils.escapeSql(name) %>' and address='<%=StringEscapeUtils.escapeSql(address)%>' ;
</sql:query>
...
<div class="col-sm-4">FIR </div>
 <div class="col-sm-4" style=""><p style="color: red;text-align: left"><a href="viewFir.jsp?fir_id=<c:out value="${result.rows[0].fir_id}"/>"><c:out value="${result.rows[0].fir_id}"/></a></p>
 </div></div>
<div class="row">
