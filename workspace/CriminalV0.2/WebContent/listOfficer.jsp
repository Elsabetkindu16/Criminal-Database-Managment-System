...
<h1 style="color: red">ALL Officers</h1>
<table cellpading="5px" style="width:60%; height: 100px;text-align: left";>
  <sql:query var="result93" dataSource="${snapshot}">select * from investigation_officer ;</sql:query>
   <tr><th>SNO</th><th>Officers</th></tr>
   <% i=1; %>
   <c:if test="${result93.rowCount!=0}">
   <c:forEach var="row" items="${result93.rows}">
   <tr><td><%= i++ %></th><td><a href="viewOfficer.jsp?id=<c:out value="${row.id}"/>">Investigation Officer&nbspId:<c:out value="${row.id}"/>&nbspName:<c:out value="${row.name}"/></a>
<br>
<br></td></tr>
   </c:forEach>
   </c:if>
